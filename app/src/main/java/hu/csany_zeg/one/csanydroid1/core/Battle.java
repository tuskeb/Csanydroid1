		...
		for (final Method method : mListener.getClass().getMethods()) {
			...
			if (mState == annotation.value()) {
				...
				try {
					...
					Log.e(TAG, "invoke failed (state: " + mState + ")");
					e.printStackTrace();
				}
...
*/
	...
	private void releaseHeroes() {
		...
		for (Hero hero : heroes) {
			hero.getOwner().send(new Player.Message(Player.ACTION_REMOVE_HERO_FROM_BATTLE, hero) {
				@Override
				public void extra(Parcel m) { }
			});
		}
